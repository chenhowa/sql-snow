...
spec = describe "Testing parsing functions" do
    ...
    describe "for sequences" do
        it "anyToken" do 
            P.runParserT (S.fromFoldable [ 1 ]) Parser.anyToken `shouldEqual` Identity (Either.Right 1)
        it "satisfy" do 
            P.runParserT (S.fromFoldable [ 1 ]) (Parser.satisfy \token -> token == 1) `shouldEqual` Identity (Either.Right 1)
        it "token" do 
            P.runParserT (S.fromFoldable [ 1 ]) (Parser.token 1) `shouldEqual` Identity (Either.Right 1)
        it "eof" do 
            P.runParserT (S.fromFoldable []) (Parser.eof :: P.ParserT (S.Seq Int) Identity Unit) `shouldEqual` (Identity (Either.Right unit))
        it "oneOf" do 
            P.runParserT (S.fromFoldable [2]) (Parser.oneOf [1, 2]) `shouldEqual` Identity (Either.Right 2)
        it "noneOf" do
            P.runParserT (S.fromFoldable [3]) (Parser.noneOf [1, 2]) `shouldEqual` Identity (Either.Right 3)